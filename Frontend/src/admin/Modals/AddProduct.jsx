...
function AddProduct({isOpen, onClose}) {
  const [formData, setFormData] = useState({
    ...
    style: "",
    pattern: "",
    features: ["Durable fabric", "Easy to install"], 
    dimensions: {
      length: 0,
      width: 0,
      unit: "inches",
  ...
  const categories = [
    { value: "Ready-Made Curtains", label: "Ready-Made Curtains", color: "bg-blue-100 text-blue-800" },
    { value: "Bed & Bath", label: "Bed & Bath", color: "bg-green-100 text-green-800" },
    { value: "Sofa Upholstery", label: "Sofa Upholstery", color: "bg-purple-100 text-purple-800" },
    { value: "Curtains & Blinds", label: "Curtains & Blinds", color: "bg-orange-100 text-orange-800" },
    { value: "Wallpaper", label: "Wallpaper", color: "bg-pink-100 text-pink-800" },
  ];
  ...
  const handleDrop = async (e, isAdditional = false) => {
    ...
    if (files.length > 0) {
      if (!isAdditional) {
        setFormData(prev => ({ ...prev, mainImageUrl: files[0] }));
      } else {
        setFormData(prev => ({
          ...prev,
          additionalImageUrls: [...prev.additionalImageUrls, ...files]
        }));
      }
    }
  };
...
const handleAdditionalImagesChange = async (e) => {
  ...
  if (files.length > 0) {
    ...
    setFormData((prevData) => ({
      ...
      additionalImagesPreview: [...prevData.additionalImagesPreview, ...dataUrls], 
    }));
  }
};
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 md:p-4">
      <div className="bg-white md:rounded-2xl shadow-2xl w-full md:max-w-4xl h-screen md:max-h-[95vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-indigo-100 text-sm">Fill in the details below to add a new item</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-800 flex items-center"><Camera className="w-6 h-6 mr-3 text-pink-600" />Images & Media</h2>
              ...
              >
                {formData.mainImageUrl ? (
                  <div className="relative w-full h-48 rounded-lg overflow-hidden mb-4 group">
                    <img src={formData.mainImagePreview} alt="Main Preview" className="object-contain w-full h-full" />
                    <button
            ...
            <div>
              ...
              <div
                className={`relative border-2 border-dashed rounded-xl p-6 text-center transition-all duration-300 ${
                  additionalDragActive ? 'border-pink-400 bg-pink-50' : 'border-gray-300 hover:border-pink-400 hover:bg-pink-50'
                }`}
              ...
              >
                ...
                <p className="text-sm text-gray-500 mt-2 hidden md:block">Multiple images supported</p>
              </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                    {formData.additionalImagesPreview.map((url, index) => (
                      <div key={index} className="relative group rounded-lg overflow-hidden shadow-sm border border-gray-200">
                          <img src={url} alt={`Additional Preview ${index + 1}`} className="object-cover w-full h-24" />
                          <button
